...
export const Artist = () => {
  ...
  const [status, setStatus] = useState<IStatus>({albums: {isLoading: true, isError: false}, tracks: {isLoading: true, isError: false}});
  ...
  const message = status.albums?.isLoading || status.tracks?.isLoading ? "Loading..." :
  status.albums?.isError || status.tracks?.isError ? "Error fetching data!" : null;
